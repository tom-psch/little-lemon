...
export default function DishCard (props) {
    ...
    return (
        <article className={classes.specialCard}>
            <img height={185} width={265} className={classes.dishPic} src={require(`./components/assets/${a}`)} alt={"A picture of a " + props.item.title}></img>
            <h2>{props.item.title}</h2>
            <h3>{props.item.price}</h3>
            <h4>{props.item.desc}</h4>
            <Link to="/order" aria-label="Click to order online" className={classes.order}>
                <p>Order a delivery</p>
                <img width={25} className={classes.bell} src={bell} alt="An icon of a bell"></img>
            </Link>
        </article>
    )
}
